...
def get_teams():
    ...
    data = api_get(f"sites/{SITE_ID}/teams.json")
    teams = {}
    ...
    # Match senior teams using exact name match first, then fallback
    for senior in SENIOR_TEAMS:
        # First try exact match
        for team in all_teams:
            name = team.get("team_name", "").strip()
            if name.lower() == senior.lower():
                teams[senior] = str(team["id"])
                logger.info(f"  Exact match: '{name}' -> {senior} (ID: {team['id']})")
                break

        # If no exact match, try contains but exclude Womens/Sunday/Ladies
        if senior not in teams:
            exclude = ["women", "sunday", "ladies", "girl", "junior", "under", "colts"]
            for team in all_teams:
                name = team.get("team_name", "").strip()
                name_lower = name.lower()
                if senior.lower() in name_lower and not any(ex in name_lower for ex in exclude):
                    teams[senior] = str(team["id"])
                    logger.info(f"  Partial match: '{name}' -> {senior} (ID: {team['id']})")
                    break

    logger.info(f"  Final team mapping: {teams}")
    return teams
